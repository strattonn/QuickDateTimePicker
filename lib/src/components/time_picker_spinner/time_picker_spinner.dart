...
class TimePickerSpinner extends StatelessWidget {
          ...
          if (state is TimePickerSpinnerLoaded) {
            ...
            return LayoutBuilder(
              builder: (context, constraints) {
                ...
                final int hourColumns =
                    _hourColumns(constraints.maxWidth, isMobile, minuteColumns);
                ...
                const double hoursSpacing =
                    3.0; // Grid mainAxisSpacing for hours
                const double minutesSpacing =
                ...
                final double totalGapsAndPadding =
                    ...
                    ((hourColumns > 1 ? (hourColumns - 1) * hoursSpacing : 0) +
                        ...
                        ((minuteColumns > 1
                                ? (minuteColumns - 1) * minutesSpacing
                                : 0) +
                            minutesPaddingH);

                // Shared tile size (wide layout) — divide remaining width across all columns
                final double usableForTiles =
                    (constraints.maxWidth - otherFixedWidth) -
                        totalGapsAndPadding;
                final double tileSizeForWide = usableForTiles > 0
                ...
                final double hoursWidth = hourColumns * tileSizeForWide +
                    (hourColumns > 1 ? (hourColumns - 1) * hoursSpacing : 0) +
                    hoursPaddingH;

                final double minutesWidth = minuteColumns * tileSizeForWide +
                    (minuteColumns > 1
                        ? (minuteColumns - 1) * minutesSpacing
                        : 0) +
                    minutesPaddingH;
                ...
                final bool isNarrow =
                    isMobile || constraints.maxWidth < requiredHorizontalWidth;
                          ...
                          return ElevatedButton(
                            onPressed: isDisabled
                                ...
                                : () {
                                    if (!is24HourMode) {
                                      final hourOffset =
                                          (datetimeBloc.state.dateTime.hour >=
                                                  12)
                                              ? 12
                                              : 0;
  ...
  int _hourColumns(double maxWidth, bool isMobile, int minuteColumns) {
    if (isMobile) {
      return minuteColumns == 4 ? 4 : 6;
    }
    return 4;
  }
}
